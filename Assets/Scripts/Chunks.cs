...
namespace Assets.Scripts
{
    public class Chunks
    {
        public struct Chunk
        {
            ...
            public Chunk(float2 position, int resolution, int length, Material material = null, Action<Chunk> onCreate = null)
            {
                ...
                NativeArray<float3> vertsArray = new NativeArray<float3>(resSquared, Allocator.TempJob);
                NativeArray<float3> normalsArray = new NativeArray<float3>(resSquared, Allocator.TempJob);
                NativeArray<float2> uvsArray = new NativeArray<float2>(resSquared, Allocator.TempJob);
                NativeArray<int> trisArray = new NativeArray<int>(6 * resolution * resolution, Allocator.TempJob);
